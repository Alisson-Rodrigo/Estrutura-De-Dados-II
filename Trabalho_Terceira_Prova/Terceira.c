...
void inicializarGrafo(Grafo *grafo)
{
  int vertice_inicial;
  int vertice_final;
  for (vertice_inicial = 0; vertice_inicial < QUANTIDADE_DE_VERTICES; vertice_inicial++)
  {
    grafo->vertices[vertice_inicial].id = vertice_inicial;
    for (vertice_final = 0; vertice_final < QUANTIDADE_DE_VERTICES; vertice_final++)
...
void popularGrafo(Grafo *grafo)
{
  ...
  for (vertice_inicial = 0; vertice_inicial < QUANTIDADE_DE_VERTICES; vertice_inicial++)
  {
    for (vertice_final = vertice_inicial + 1; vertice_final < QUANTIDADE_DE_VERTICES; vertice_final++)
    {
...
void djcastra(int inicio, int fim, Aresta arestas[QUANTIDADE_DE_VERTICES][QUANTIDADE_DE_VERTICES], double *distancias, int *predecessor)
{
  ...
  do 
  {
    ...
    for (vertice_atual = 0; vertice_atual < QUANTIDADE_DE_VERTICES - 1; vertice_atual++)
    ...
    if (!(vertice_maior_confianca == -1 || distancias[vertice_maior_confianca] == INFINITO_NEGATIVO))
    {
      ...
      for (v = 0; v < QUANTIDADE_DE_VERTICES; v++)
      {
